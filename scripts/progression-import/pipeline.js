...
function calcCost(dist) {
    ...
    for (const v of Object.values(dist)) {
        if (typeof v === 'number' && v > 0)
            cost += calculateCategoryCost(v);
    }
    return cost;
}
function distributionsMatch(a, b) {
    if (!a || !b)
        return false;
...
for (const card of rawData) {
    ...
    if (record.verificationStatus === 'INVALID') {
        ...
        record.verificationStatus = 'SOURCE_ONLY';
        ...
        if (match) {
            record.verificationStatus = 'VERIFIED_CROSS_SOURCE';
...
console.log("\nTop Conflicts:");
const conflicts = records.filter(r => r.verificationStatus === 'CONFLICT').slice(0, 20);
conflicts.forEach(c => {
