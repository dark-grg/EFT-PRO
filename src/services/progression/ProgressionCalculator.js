...
export class ProgressionCalculator {
    static calculateTotalCost(allocation) {
    ...
    static getAvailablePoints(card) {
        if (typeof card.maxLevel !== 'number' || card.maxLevel < 1)
            return null;
        return Math.max(0, (card.maxLevel - 1) * 2);
    }
    static applyAllocationToStats(card, allocation) {
    ...
    static calculateFinalOVR(card, finalStats) {
        if (!card.position || !card.baseStats)
            return null;
        try {
    ...
    static validateAllocation(card, allocation) {
